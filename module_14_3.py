...
async def get_buying_list(message):
    ...
    for number in range(0, 4):
        await message.answer(f'Название: Product{number + 1} | '
                             f'Описание: продукт на основе цветка {product_description[number]} '
                             f'| Цена: {(number + 1) * 100}')
        with open(f'flowers{number + 1}.jpg', "rb") as img:
...
async def start_messages(message):
    await message.answer('Привет! Я бот, помогающий Вашему здоровью. Если хотите узнать количество килокалорий, '
                         'которое нужно получать Вам для сохранения нормального веса или для оптимального похудения, '
                         'нажмите на кнопку "Рассчитать".', reply_markup=start_kb)
